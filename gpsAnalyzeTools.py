...
class gpsAnalyzeTools:
    def __init__(self,listPerDay):
        ...
        for pt in listPerDay:
            pt = pt.split('|')
            self.selectedLon.append(float(pt[1]))
            self.selectedLat.append(float(pt[2]))
            self.selectedTime.append(time.mktime(time.strptime('2016' + pt[0], '%Y%m%d%H%M')))
    ...
    def filterRangeBeijing(self):
        for i,pt in enumerate(self.selectedPts):
            ...
            if (116.1 < float(pt[1]) < 116.7 and 39.7 < float(pt[2]) < 40.2):
                self.marks.append(i)
    ...
    def filterSpeed(self,speedMax = 120,distMin = 0.1):
        ...
        self.selectedLat = [self.selectedLat[i] for i in self.marks]
        self.selectedLon = [self.selectedLon[i] for i in self.marks]
        lentmp = len(self.selectedPts)
        print 'origin'+ str(lentmp)
        self.selectedPts = list(set(self.selectedPts))
        print 'set' + str(len(self.selectedPts))
        if(lentmp > len(self.selectedPts)):
            self.selectedLon = []
            self.selectedLat = []
            for pt in self.selectedPts:
                ptTmp = pt.split('|')
                self.selectedLon.append(float(ptTmp[1]) )
                self.selectedLat.append(float(ptTmp[2]) )
